...
LibraryStatement::LibraryStatement() {
	...
	this->statement_type = kLibraryStatement;
	...
	this->statement_type_description = kLibraryStatementDesc;
	// scope
...
LibraryStatement::LibraryStatement(const LibraryStatement& orig) {
	this->id = orig.id;
...
LibraryStatement* LibraryStatement::GetCopy() {
	return new LibraryStatement(*this);
}
...
std::string LibraryStatement::ExportCircuitStatement(std::string indentation){
	...
	// ...
	// library [libraryName]
	std::string cs = indentation + kCommentWord1 + "Start of " + kLibraryWord + kDelimiter + master_name;
	cs += kEmptyLine + indentation + kLibraryWord + kDelimiter + master_name;

	//export children
	for(std::vector<Statement*>::iterator it_children = children.begin();
	 it_children !=  children.end(); it_children++){
		cs += kEmptyLine + kTab + (*it_children)->ExportCircuitStatement(indentation + kTab);
	}
...
bool LibraryStatement::ParseLibraryStatement( Statement& global_scope_parent, 
		...
		const bool permissiveParsingMode ){
	#ifdef PARSING_VERBOSE
		log_io->ReportPlain2Log( "parsing library: '" + statementCode + "'" );
	#endif
		...
		if(statementCode.compare(kEmptyWord) != 0){
			...
			if(lineTockens.front().compare(kLibraryWord) == 0){	
				...
				if( permissiveParsingMode ){
					#ifdef PARSING_VERBOSE
						log_io->ReportRedStandard( "1-> End of file, unexpected end of Library: Forced by a permisive parsing mode." );
					#endif
					correctly_parsed = true;
				}
			}
				...
				log_io->ReportRedStandard( "2-> End of file, unexpected end of Library: Forced by a permisive parsing mode." );
			#endif
			correctly_parsed = true;
		}
		endOfFile = true;
