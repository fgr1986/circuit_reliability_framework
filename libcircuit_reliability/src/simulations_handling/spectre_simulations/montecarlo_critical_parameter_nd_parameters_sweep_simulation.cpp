...
MontecarloCriticalParameterNDParametersSweepSimulation::~MontecarloCriticalParameterNDParametersSweepSimulation(){
		...
		std::cout << "MontecarloCriticalParameterNDParametersSweepSimulation destructor. direction:" + number2String(this) << std::endl;
	#endif
	...
	for( auto const &m : analyzedMagnitudes ){
		if( m->get_analyzable() ){
			...
			if( maxCritParamValue < 1 ){
				...
				gnuplotScriptFile << "set y2range [0:"<< maxCritParamValue << "]" << std::endl;
				gnuplotScriptFile << "set cbrange [0:"<< maxCritParamValue << "]" << std::endl;
			}
			// line style
